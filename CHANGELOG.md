...
- *(docker)* Rebase native BuildKit auto mode onto 1.14.5 (#128)
...
- *(hidden)* Protect hidden enrollment in policy integrity (#122)
...
- *(hidden)* Fail closed on skipped required checks (#121)
...
- *(mutation)* Exclure les fichiers non-production du périmètre verify (#126)
...
- Bootstrap mutation scope for new Python repositories (#124)
...
- *(trusted)* Support annotated tags in policy integrity (#116)
...
- *(hidden)* Add Keryx conversation runtime oracle (#107)
...
- *(hidden)* Add Solar energy-routing oracle (#109)
...
- *(hidden)* Support annotated release tags (#114)
...
- *(hidden)* Materialize protected base separately (#113)
...
- *(trusted)* Add central hidden evaluator framework (#103)
...
- *(trusted)* Add base-anchored policy integrity gate (#112)
...
- *(docker)* Reuse preinstalled binfmt handlers (#110)
...
- *(mutation)* Add trusted Stryker gate for Node/TS (#106)
...
- *(trusted)* Separate blocking gate from evidence reporting (#104)
...
- *(quality-report)* Fallback to writable uv cache on ARC (#99)
