...
class QualityReportTests(unittest.TestCase):
    ...
    def test_transient_github_api_errors_are_retried(self):
        ...
        with patch.object(
            quality_report.urllib.request,
            "urlopen",
            side_effect=[transient, transient, io.BytesIO(b"{}")],
        ) as mocked, patch.object(quality_report.time, "sleep") as sleep:
            with quality_report._urlopen_with_retry(request) as response:
                self.assertEqual(response.read(), b"{}")

        self.assertEqual(mocked.call_count, 3)
        self.assertEqual(sleep.call_count, 2)

    def test_forbidden_github_api_error_is_not_retried(self):
        request = quality_report.urllib.request.Request("https://api.github.com/test")
        forbidden = quality_report.urllib.error.HTTPError(
        ...
        with patch.object(
            quality_report.urllib.request, "urlopen", side_effect=forbidden
        ) as mocked, patch.object(quality_report.time, "sleep") as sleep:
            with self.assertRaises(quality_report.urllib.error.HTTPError):
                quality_report._urlopen_with_retry(request)
